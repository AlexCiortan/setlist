...
- **`SETLIST_SKIP_TRUNK_AUDIT=1` turns off the whole of `pre-push`, not just the audit.** It is read before the ref loop, so the push-time secret and em-dash scan does not run either, and the only thing printed is "trunk audit skipped". Measured: a push refused `SLH-SECRET` succeeds with that variable set, and the secret lands on the remote. It was undocumented outside the hook's own header until the 2026-08 claims audit found it. Treat it as a full bypass of the push-time boundary, with the same weight as `--no-verify`.

  **Status: fix scheduled for the next release cycle**, where the variable is narrowed to the audit it names and the push-time scan keeps running.

- **Secret and style scanning is best-effort early warning, not a guarantee.** Treat any secret that reached a commit as COMPROMISED and rotate it, whatever these hooks reported. This limitation is written at this strength because the claim above it was narrowed three times and falsified three times under this release's own adversarial review, and the mechanism is what decides which sentence is true. The scanner reads the output of `git diff`, which is a RENDERING the repository controls, so the holes are structural rather than a matter of pattern coverage: a `.gitattributes` entry marking a path `-diff` or `binary` makes git emit no added lines at all, and a live-shaped secret then reaches a remote at exit 0 by the ORDINARY commit-and-push path (measured under adversarial review). Beyond that: git fires no hook for `cherry-pick`, for `rebase`'s intermediate commits, or for the apply step of `git am`, so those are read at push rather than at commit, and **what is read at push is an ENDPOINT DIFF, not a walk of the commits**. `git diff A..B` compares two trees, so a value added in one commit of the pushed range and removed before the tip is never seen, while every object in the range is still transferred and readable on the remote with `git log -p`. Measured: the same secret at the tip is refused `SLH-SECRET`, and one commit further back it pushes at exit 0; the first push of a branch has no remote counterpart to diff against and derives its range from the local trunk, which is EMPTY when the ref being pushed is itself the trunk, and skipped entirely when no merge-base exists; pushing a TAG is not a branch update and is not scanned at all; the diff reader drops the `+++ b/path` header with a pattern that also drops any SOURCE line beginning `++`, so `++api_key = "..."` commits and pushes silently where the same line without the prefix is refused (found by adversarial review and classified scan-only, since trunk discipline still refuses the merge); and the pattern set is a first cut rather than a complete secret detector. What the scan is good for is catching an accident early on the ordinary path. It is not a control you can put between a secret and a remote.

  **Status: partly scheduled, partly structural.** Four of the holes above are scheduled for the next release cycle: the endpoint diff, the empty first-push range, the unscanned tag push, and the `++` line-anchor. The two that are not are structural and stay: a `.gitattributes` entry that suppresses the diff, and the commands git fires no hook for. The sentence above the list is the one to trust either way.

- **`git merge --ff-only` and `git merge --ff` skip the merge hooks.** A fast-forward creates no merge commit, so there is no `pre-merge-commit` for git to run, and an explicit `--ff-only` beats the `merge.ff=false` Setlist sets. So does a bare `--ff`, which is the same hole under a flag name this list used to leave out: measured, it fast-forwards a spec branch with no Closing report onto the trunk and fires neither hook. Unlike `--no-verify` neither looks like a bypass, which is why they are called out here: they are normal preference flags and some GUI clients pass them for you. The consequence is the same two-layer disagreement the next bullet documents for `--squash`, and it leaves the trunk UNPUSHABLE: a fast-forward puts the spec's commits directly on the trunk with no closing merge, so the pre-push trunk audit sees role-path code that did not arrive through a spec-closing merge and refuses the push, whether or not the close was compliant. `pre-commit` allowed the commit and `pre-push` then refuses it, permanently, so close with `--no-ff` (which `/setlist:checkpoint` does) and treat fast-forwarding a close onto the trunk as unsupported. The suite is separately not re-run by the audit, but that never comes into play on this route because the audit refuses the fast-forwarded close before any gate command would run (found by adversarial review).

  **Status: fix scheduled for the next release cycle**, jointly with the `--squash` bullet below: the two layers are to be taught to agree rather than to disagree politely.

- **`git merge --squash` does not work in a Setlist instance, and the error does not say so.** `merge.ff=false` implies `--no-ff`, and git refuses `--squash` in combination with it: `fatal: options '--squash' and '--no-ff.' cannot be used together`, on every squash merge, whether or not the branch could have fast-forwarded. The message names neither Setlist nor the setting, so there is nothing to search for. The workarounds are `git -c merge.ff=true merge --squash <branch>` for a one-off, or dropping the setting for that command. **Neither leaves you with a pushable trunk, and this list recommended one of them until 2026-08-04 without saying so.** A squash has no second parent, so the trunk audit can never see the branch the work came from: `pre-commit` runs the full close verification and ALLOWS the commit, and then `pre-push` reports it as feature code committed directly to the trunk and refuses the push, permanently. The two layers disagree about a route the docs were prescribing. Until they are taught to agree, close a spec with `--no-ff` (which is what `/setlist:checkpoint` does) and treat squash-merging onto the trunk as unsupported. The setting stays because a plain fast-forward merge fires no git hook at all, and that hole is worth more than the convenience.
...
- **The staged-content scans read every staged line.** The em-dash and secret scans do not know vendored third-party code, quoted external text, or test fixtures with dummy credentials from your own new writing. **Splitting the commit does not help, and this list said it did until 2026-08-04.** The scans read every added line of the index, so isolating the foreign content isolates it WITH the scanner rather than away from it: a commit whose staged set is only the vendored file is refused identically. `SETLIST_SKIP_HOOKS=1` exempts the COMMIT and NOT the push, so it does not leave you with a pushable branch: measured, the exempted commit succeeds and the next `git push` is refused `SLH-EMDASH` whether or not the variable is set, because `pre-push` does not read it. This list recommended that procedure without saying so until the 2026-08 claims audit measured it. What actually works today is to keep foreign content out of the scanned paths, or to accept that the branch carrying it needs `--no-verify` on the push, which is a decision to own rather than a procedure this project endorses. Do not edit third-party content to satisfy a style gate. Path-scoping the scans is not built; when it is, this bullet changes.

  **Status: fix scheduled as the next release cycle's one feature**, which is what path-scoping the scans amounts to.

- **A spec that QUOTES the closing-report template inside a fence is refused an ordinary commit.** This one refuses honest work rather than permitting dishonest work, which makes it the most annoying entry on this list and the one most likely to meet you first. The `pre-commit` hook decides whether a commit changes a spec's lifecycle state by reading the raw staged diff for a `Status:` line or a `Closing report` heading, and it does not strip fenced blocks. So a spec that documents the close format for a reader, changing no state of its own, looks identical to a real close: the commit is refused `SLH-STATUS-MISSING` and asks you to stage `specs/STATUS.md` for a transition that is not happening. Measured on the shipped bytes with controls in both directions: an ordinary spec edit commits clean, a real close without the inventory row is refused, and the fenced quotation is refused the same way. **The workaround is to keep the quotation out of `specs/*.md`**, because that is the only path the detector reads: the same fenced template in a `docs/` file, or in `specs/TEMPLATE.md`, commits clean. Failing that, `SETLIST_SKIP_HOOKS=1` on that one commit. **What does NOT work is staging `specs/STATUS.md` alongside it**, which is the obvious move and was written into this bullet before it was measured: `git add` on an unmodified inventory file stages nothing, so the check still sees no row and refuses identically. Only a real edit to that file satisfies it, and inventing an inventory change for a spec that is not transitioning is worse than the refusal. Three other readers in this release already fence-strip; this fourth one is the odd man out, and aligning it is the fix.

  **Status: fix scheduled for the next release cycle**, where the detector is aligned with the three sibling readers rather than given a fourth private rule. This bullet is DELETED by the commit that fixes it.
