...
**Setlist** is spec-driven development for Claude Code. You write the setlist, Claude Code plays it, and git hooks stamped into your repository refuse work that skipped the process. You direct rather than type.

**Who it is for.** One developer or a small team who would rather review than type. You run Claude Code, you want every feature to arrive through a spec you approved, and you want that to still hold after the chat is gone, the context is compacted, or the model is swapped. If you want an agent that ships ten things in parallel and you review later, this is the wrong tool. The "Why one spec at a time" section below says why.

**What you get in the first hour.** `/setlist:new` interviews you, proposes the stack and the core data model for your approval, stamps the hooks, and writes your first specs. `/scaffold` makes the first commit. From then on each feature is one spec, one branch, and one reviewed close. A merge that closes no spec is refused by git itself, in your clone and on the pull request.
...
- **What an agent says it did is not evidence.** A model can state an intent, or a finished step, without issuing the call that does it, and current models are documented to fail that way. Nothing Setlist enforces reads that account: the git hooks judge repository state and not claims, the `qa-pass-1` block refuses any line that is not a verdict, the Stop hook refuses to end a turn that leaves a spec change unstaged, and a check that could not run has not passed.
...
- **The repository is the memory.** Every durable decision lands in a version-controlled file: an ADR (architecture decision record) log with an index, a bounded status file, per-session journals, and living architecture diagrams at three altitudes, whose field is gate-checked on the merges that CLOSE a spec (the checks iterate over the specs a change closes, so a chore merge carrying role-path code does not reach them; verified by adversarial review). Any session, on any model, re-grounds by reading the repo. Chats are disposable; the repo persists.
...
| `/setlist:gate` | Runs a stage-gate transition when your project moves between roadmap stages: verifies the stage's exit criteria leg by leg, reconciles the next stage's backlog, and specs its first entry. A failed gate is a finding, not a formality. (Distinct from the mechanical close checklist that `/setlist:checkpoint` runs.) | You only |
| `/setlist:checkpoint` | Spec-scoped Git: opens the spec branch, surveys and commits with secret and style checks, runs the close checklist, merges `--no-ff`. | You, or the model when about to commit |
...
The plugin also carries five **reference skills**: context the model loads by itself at the moment it applies, never commands you type.
...
| `diagrams` | A diagram is being drawn or synced | The routing test that picks a type, the drawing and truthfulness rules, altitudes and layout, node evidence, and five type references loaded on demand |
...
Since 2.7.0 a diagram is a set of claims the close checks, not a picture the close asks about. Two things opt you in: a file under `docs/diagrams/` carrying the four-line header, or a declared `diagram_command`. With neither, every reader behaves exactly as it did before. Each item below names the boundary bullet that says where it stops, so this section and the Known-limitations list never disagree.

- **Three diagram altitudes, one file per view.** Context (`docs/diagrams/context.md`), containers (the L2 block in `steering/structure.md`, or in whichever document your readers actually open) and components (one file per role path under `docs/diagrams/components/`). Every file under `docs/diagrams/` opens with four lines: what it shows, its altitude, the spec that last synced it, and what it encodes. A diagram drawn inline somewhere that is not a diagram file folds those four into the prose around it. Boundary: *A diagram check compares a claim to a diff and a lockfile to a command's output.*
- **The diagram field names files, and the close compares it to the diff.** `Architecture diagram: updated (docs/diagrams/components/auth.md)` is refused when a named file is not in the closing commit, or when it names nothing at all, and `no impact` is refused when the commit touched a diagram. The merge hook, the trunk audit and the forge check all ask it. Boundary: the same bullet.
- **Node evidence, resolved against the tree.** Every container and component node is named by the path it draws, written in the node's own label; a path that does not exist at the close is refused for the closing spec's own nodes and reported for older ones, and a name that is not path-shaped comes back printed rather than silently skipped. Boundary: *A drawn name is verified only when it is path-shaped.*
- **A generated view as a lockfile, opt-in.** Declare `diagram_command` in `.claude/sdd.json` and the file you commit under `docs/diagrams/generated/` must equal what that command prints, at every close. Setlist ships no extractor; the command is yours. Boundary: the first bullet.
- **Render validation at the forge.** The forge check parses every Mermaid block under a pinned Mermaid version and refuses one that does not parse. The stamped workflow's parser step fails the job rather than letting the check pass without a parser, and verifies itself in both directions first. Boundary: *The render check runs where a renderer runs.*
- **The Design sketch, and the `diagrams` skill.** A spec can draw the intended change in the type the skill's routing test picks; it is what you approve and what the agent builds to, and `/setlist:checkpoint` absorbs what survives into the living diagrams at the close. The skill carries the drawing rules and five type references it loads only when that type is being drawn.
- **The baseline chore, for an instance that upgrades.** `/setlist:upgrade` creates nothing and redraws nothing: it reports, in one line, that you are drawing at one altitude. `chore/diagram-baseline` is the opt-in, and it is the one place in Setlist where scanning the code and drawing fresh is allowed, under your review file by file. After it, diagrams are edited in closing commits and a wholesale re-scan is refused.
- **The forge check reads the strict setting.** Under `forge` custody a trunk that requires the check but does not require branches to be up to date before merging is now refused by name, because two pull requests can otherwise both be green against a stale base. Under every other custody it is reported beside the review count. Boundary: *The forge check governs the merge button only where the trunk requires it.*

## Less to run, since 2.8.0

The session layer is smaller since 2.8.0, and every piece that left is named with what now answers its question. Each item below that has a boundary names the bullet that says where it stops, so this section and the Known-limitations list never disagree.

- **The two Bash advisory gates are removed.** The commit gate and the close gate, 1,735 lines of frozen command-text parser whose verdicts Claude Code does not deliver to the agent on `allow`, are no longer stamped. Every question they asked is answered by the git hooks, which refuse where the gates only warned, or by `/setlist:validate` (a declared git identity is compared at the health check), or was a question about a command's text that leaves with the parser that asked it. Boundary: *The session layer's one command reader is a frozen text parser, and a short list of spellings reads a command wrongly.*
...
One container view of a project Setlist has stamped: which layer runs each check, and which two files they all read, `.claude/sdd.json` and `.claude/status.json`. Two boxes, one advisory hook, and one rule; the one refusal above the boxes reads none of the files the picture is about. `.claude/hooks/scope-hook.sh` is the advisory hook: it reads `.claude/sdd.json`, reports on a write, and permits it. The box holding `.githooks/pre-commit`, `.githooks/pre-merge-commit`, `.githooks/pre-push` and `.githooks/setlist-hook-lib.sh` refuses, in your clone, on state git has already resolved. The box holding `.github/workflows/setlist-forge-check.yml` and `.claude/hooks/forge-check.sh` asks the same questions on a runner no committer controls. `.claude/hooks/trunk-audit.sh` sits outside both boxes because `.githooks/pre-push` and `.claude/hooks/forge-check.sh` both run it, the same script over different ranges.
...
**What the arrows assert.** Every refusal that stands between unreviewed work and a shared trunk comes from `.githooks/pre-commit`, `.githooks/pre-merge-commit`, `.githooks/pre-push` through `.claude/hooks/trunk-audit.sh`, or `.claude/hooks/forge-check.sh`. The arrow from `.claude/hooks/scope-hook.sh` to `.claude/sdd.json` is a read that ends in a report, never in a refusal. The four arrows into `.githooks/setlist-hook-lib.sh` are one set of predicates, sourced by `.githooks/pre-commit`, `.githooks/pre-merge-commit`, `.githooks/pre-push` and `.claude/hooks/forge-check.sh` alike. The arrow from `.claude/hooks/forge-check.sh` to `.claude/sdd.json` reads the config at the pull request's head, and the base that check judges the merge against is the trunk. The arrow from `.github/workflows/setlist-forge-check.yml` to `.claude/hooks/forge-check.sh` passes the workflow step only when the check prints its PASS token, so anything else refuses. Every arrow is a call or a read you can grep for inside these files. Every node is stamped by `/setlist:new`, and every node but `.claude/status.json` is stamped by `/setlist:upgrade` too: an upgrade never writes `.claude/status.json`, because an instance that predates the record opts in through `/setlist:checkpoint`'s one-time transcription, which prints what it would record and writes only after you confirm the list. Absent `.claude/sdd.json` on the checked-out branch, every node that runs exits 0 in silence: the project has not opted in.

Three of the four session hooks are not in the picture. The bypass deny is the one refusal above the boxes, and it reads only the command it is given; the other two judge session events rather than commands: session re-grounding, which injects at session start, and the Stop hook, which does refuse, ending no turn that leaves a spec or `specs/STATUS.md` change unstaged. What the picture does not draw at all is the ways around it, which are the next section.
...
- **The scans read this project's own index.** A commit aimed at another repository (`git -C nested commit`) runs that repository's hooks and is not scanned; your own staged content always is, through whatever index git commits. [Full text.](LIMITATIONS.md#own-index)
...
- **A diagram check compares a claim to a diff and a lockfile to a command's output; it does not judge whether a drawing is true of the code.** The field checks establish that the files the closer named are the files the commit touched, the node check that every drawn path exists, and the lockfile that the generated view equals the command's output; whether the picture is true of the mechanism is what review establishes. [Full text.](LIMITATIONS.md#diagram-checks)
- **A drawn name is verified only when it is path-shaped, and everything else is reported rather than checked.** A name with a slash and no whitespace is resolved against the tree; anything else is printed as unverified, along with three Mermaid shapes the reader does not open at all. [Full text.](LIMITATIONS.md#path-shaped-names)
- **The render check runs where a renderer runs, and the stamped workflow fails its job rather than passing without one.** Inside the stamped workflow the Mermaid parser step succeeds or fails the job; run anywhere else the check reports that it could not parse and refuses nothing on that ground, and local git hooks never render. [Full text.](LIMITATIONS.md#render-check)
- **The session layer's one hard deny is defeated by ordinary variable expansion, so read it as a nudge and never as a boundary.** `git commit -m ${MSG} --no-verify` is allowed in silence, because an unquoted expansion before the flag defeats the lexer behind `CM-BYPASS-SPELLED`; crafted spellings defeat it too (a wrapper before the escape variable, a redirection or a line continuation before the flag among them), it is silent on a broken jq or awk, the git hooks are unaffected, and the parser freeze is why none is repaired. [Full text.](LIMITATIONS.md#hard-deny-expansion)
- **The session layer's one command reader is a frozen text parser, and a short list of spellings reads a command wrongly.** Since 2.8.0 the bypass deny's lexer is the only session code that reads command text; it is frozen since 2026-08-04 and pinned by digest, changed once by decision to stop the false denials its 2.8.0 review found, a short list of spellings still misleads it, and for all but one item the git hooks judge the same operation correctly afterwards. [Full text.](LIMITATIONS.md#parser-spellings)
...
- **An alias of a spec branch is judged by what its merge closes, never by the name it was merged under.** The session gate that followed an alias by commit left in 2.8.0; the merge hook and the trunk audit judge a merge by the spec rows it flips to CLOSED, so an alias of unclosed work is refused at the merge (`SLH-CLOSES-NO-SPEC`) whatever it is named. [Full text.](LIMITATIONS.md#alias-identity)
- **The wiring check recognises only the command spellings `settings.json.tmpl` ships.** A hand-rewritten hook entry that genuinely runs the stamped file is reported `NOT WIRED`, and the check is a wiring inventory rather than a proof: it matches the command's first word, so a spelling that runs the stamped file and throws its verdict away can be certified as wired. [Full text.](LIMITATIONS.md#wiring-spellings)
...
- **`git reset --hard <spec-branch>` moves the trunk onto unclosed work outright.** Same silence before the push, same refusal at push, same qualification for a declaring close. [Full text.](LIMITATIONS.md#reset-route)
...
| `.claude-plugin/`, `skills/`, `templates/`, `scripts/` | The plugin (`setlist`): the marketplace and plugin manifests, the seven `/setlist` command skills and five reference skills, the instance templates including the stamped hooks, and the scripts that stamp an instance, extract a Part, and refresh an existing instance's enforcement files. All of it is a binding of the edition document. |
