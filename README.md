...
- **Sideways routes to the trunk.** The close gate reads the merge command; content can still reach the trunk through commands it does not parse: `git cherry-pick` of spec-branch commits, `git pull <remote> spec/...` (a fetch-and-merge the gate does not intercept), merges performed on a detached HEAD (no current branch, so the gate never sees a trunk target), and merging a spec branch under a second name (`git branch tmp spec/0001-x && git merge tmp`, where the ref the gate is given is genuinely not a spec branch). These are named rather than parsed because each is a deliberate spelling of "go around the gate", and a visible edge is more honest than a half-parser that would miss the next spelling. Every one of them is caught by the trunk audit below, which is why the two layers exist: the gates stop things as they happen and can be spelled around, the audit reads the history afterwards and cannot.
