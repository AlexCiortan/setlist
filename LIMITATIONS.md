...
**A third route to the same effect, measured by the 2.8.0 adversarial review.** `pre-push` finds the trunk audit through `CLAUDE_PLUGIN_ROOT` before the instance's own stamped copy and does not check what it found, so a shell that sets that variable to a directory holding its own `scripts/trunk-audit.sh` runs that script instead, and a push of unclosed work can then succeed with nothing printed. The variable is not set in an ordinary Claude Code session's shell (Claude Code sets it for plugin hook and skill processes, pointing at the real plugin), so meeting this takes a deliberately planted directory, and the bypass deny does not recognise it.
...
#### The session layer: advisory, and its one parser frozen

<a id="hard-deny-expansion"></a>
- **The session layer's one hard deny is defeated by ordinary variable expansion, so read it as a nudge and never as a boundary.** The bypass deny carries the session layer's single hard refusal, `CM-BYPASS-SPELLED`, which exists to stop a command that would disarm the git hooks; `git commit -m ${MSG} --no-verify` is allowed in silence, because an unquoted expansion before the flag defeats the lexer that looks for it.
  ...
  **The same reader refuses one shape of ordinary work, and the fix is one keystroke.** A heredoc written with NO space before the operator (`cat<<EOF`) is still denied when its body mentions a disarming spelling, because the reader only recognises the operator when it starts a word. The spaced spelling (`cat << EOF`) is read correctly and allowed only when `cat`, `tee` or `git` is the first word of its command: a wrapper or an assignment before it (`env cat << EOF`, `LC_ALL=C cat << EOF`) is still denied, measured by the 2.9.0 adversarial review and disclosed for the same reason. Writing the space, and dropping the prefix, is the whole of the workaround; nothing about the git hooks changes either way. Measured 2026-09-16 by this release's own cold review, disclosed rather than repaired because reaching the no-space form means changing how words are split rather than which spellings are listed.

<details><summary>The history and the measurements</summary>

This bullet is written at this strength because the alternative is worse than the hole. The gate's other verdicts have been advisory in mechanism since 2026-08-04; this one deny is the exception, and it is the one a reader is most likely to mistake for a boundary. **What defeats it is not exotic.** `-m ${MSG}` is how a shell script writes a commit message, so a developer meets this while doing nothing unusual, which is why the sentence above does not say "crafted spellings evade it". Crafted spellings evade it too, and they are the same hole rather than a second one: ANSI-C quoting (`git commit $'--no-verify'`), a global option that takes a separate value and shifts where the subcommand is found (`git --attr-source HEAD commit -n`), and `--config-env=core.hooksPath=VAR`, which moves the hooks path without tripping `CM-HOOKSPATH-MOVED`. All four were measured against the shipped bytes by the 2.7.0 adversarial review, each with a replay. The 2.8.0 adversarial review measured more of the same hole on the shipped bytes, each against a repository whose git hooks refuse: the escape variable behind a wrapper other than a bare `env` or `export` (`/usr/bin/env SETLIST_SKIP_HOOKS=1 git commit -m x`, `nice env ...`, `time ...`, `env -i ...`), a redirection such as `2>&1` placed before the flag (`git commit -m x 2>&1 --no-verify`), and a backslash-newline line continuation before `git` or the flag when the continued line starts at its first column. **None of these is repaired, and the reason is the parser freeze of 2026-08-04**, recorded in the bullet below: the guarantee moved to the git hooks, each parser repair here has historically introduced its own defect, and widening a lexer to chase spellings is the coverage chase this project has twice removed checks over. The git hooks are unaffected by every one of them: what is lost is an early refusal in the session, not the boundary at the trunk. **And on a machine where jq or awk does not work, the deny says nothing at all:** the rule is simply absent there, and because the spellings it governs are exactly the ones that disarm the git hooks, the git hooks are no fallback for it.

</details>

<a id="parser-spellings"></a>
- **The session layer's one command reader is a frozen text parser, and a short list of spellings reads a command wrongly.** Since 2.8.0 the bypass deny's lexer is the only session code that reads command text (the scope hook reads file paths): it moved verbatim, it is frozen and pinned by digest (changed once since, by decision, to stop the false denials its own 2.8.0 review found), and a short list of spellings still misleads it.
  **What to do:** Read the bypass deny as a nudge and the git hooks' refusals as the answer; keep the `Architecture diagram:` label on one line of the spec, inside the Closing report, because that one item is read the same way by the git hooks.

<details><summary>The history and the measurements, and the one spelling with a bullet of its own</summary>

**For all but one item the git hooks judge the same operation correctly afterwards.** The one exception is not a command and is recorded here because this bullet has always carried it: the architecture-diagram field is read from the whole spec rather than from the Closing report section, so a mention of the label ELSEWHERE in the spec can decide it in either direction (the readers take the FIRST matching line, ruled first-line-wins in 2.3.0), and that first-match reading is in the merge hook and the audit alike, so no later layer corrects it and it can let work reach the trunk. What the lexer misreads is measured: the spellings that defeat its one refusal are in the hard-deny bullet above. In the refusing direction the 2.8.0 adversarial review measured three classes on the shipped bytes, and the owner ruled them FIXED in 2.8.0 inside the pinned lexer, the one exception taken to the freeze: a heredoc body written through `cat`, `tee` or `git` that quotes a disarming spelling IN ITS SPACED SPELLING (`cat << EOF`), the no-space spelling (`cat<<EOF`) being still refused, a read-only search whose pattern or post-`--` operand is the spelling (`git log -S --no-verify`, `git grep -- --no-verify`), and a combined short-flag commit whose message is the spelling (`git commit -am "--no-verify"`). One refusing case remains that blocks a command a person can actually run: a `git` word that is an argument to another command, such as `echo git commit --no-verify`, is still denied although it runs no git. A second was measured by this release's own fix round and left as it stands: a search that puts the spelling in OPTION position, such as `git grep -n --no-verify` or `git log -p --no-verify`, is still denied, and git itself refuses that spelling (`error: unknown option`), so what the deny turns away there is not a command that could have run. Every spelling git accepts as a search for the token is allowed, with or without other flags, and whether the search option stands alone or is combined with others: `git grep -e --no-verify`, `git grep -n -e --no-verify`, `git grep -- --no-verify`, `git grep -n -- --no-verify`, `git log -S --no-verify` and `git log -pS --no-verify`. (A commit message that is exactly an escape spelling, such as `git commit -m SETLIST_SKIP_HOOKS=1`, was denied until the 2.7.0 leg's fix and is not any more.) Everything else is FROZEN by decision: the lexer and its corpus are closed to new spellings as of 2026-08-04, because the guarantee moved to the git hooks and each parser repair has historically introduced its own defect, and since 2.8.0 its bytes are pinned by digest, so a widening is a visible decision rather than a drift. **Through 2.7.0 this bullet described three advisory gates and listed some twenty spellings the commit gate and the close gate misread**, from a checkout run in a different repository to a quoted merge operand outside the ASCII keep-class, two of them with bullets of their own (a `<<\EOF` heredoc read as code, and `@{u}` refused as unnameable). Every one was a property of those two parsers, and all of them left with the parsers in 2.8.0, the two bullets included; the bypass deny's own lexer read heredoc bodies as commands too, a separate refusal fixed in 2.8.0 and described above. **The short form and this text used to disagree**, one saying all but one item and this one saying two, because the heredoc member had once run your gate command before an ordinary commit; that stopped in 2.6.0 and the member has left, so both layers now say one.

<a id="case-spelling"></a>
- **A role directory spelled in a different case is not seen by the session scope gate on macOS or Windows.** With `roles.src` recorded as `src`, a write to `SRC/a.txt` is the same file on a case-insensitive filesystem but does not match the role path, so the gate stays silent where `src/a.txt` is warned.
  **What to do:** Spell role paths as recorded; the audit at push is the layer that holds.

<details><summary>The history and the measurements</summary>

The trunk audit catches it: git stores the path under its on-disk spelling, so the commit is reported as feature code on the trunk and `pre-push` refuses it. Advisory layer misses, guarantee layer holds, which is the layering this release claims. **The same layering holds for a symlinked LEAF (2.4.1 review):** the scope gate resolves the directory of a written path and re-attaches the final name literally, so a write through a symlink whose target is a role-path file draws no advisory, while a symlinked role DIRECTORY is warned; the trunk audit refuses the resulting commit at push, measured. **Re-measured in 2.9.0**, when the scope hook's warning began reaching the agent: the hook's path predicate was enumerated case by case first, and both misses stand as written here; a third, a relative write path, has its own bullet below.
...
- **An alias of a spec branch is judged by what its merge closes, never by the name it was merged under.** Until 2.8.0 the session close gate identified an alias by commit, and only while a spec or chore ref still pointed at that exact commit (the 2.4.0 review measured that boundary); that gate left in 2.8.0. The git hooks never identified a close by its branch name: `pre-merge-commit` and the trunk audit ask which specs a merge flips to CLOSED, so an alias of unclosed work is refused whatever it is named.
  **What to do:** Merge the spec branch by its own name; nothing reaches the trunk by the alias route.

<details><summary>The history and the measurements</summary>

Measured for 2.8.0 on an instance armed with the stamped hooks: an alias cut at an unclosed spec branch, merged onto the trunk after the spec branch advanced, is refused by `pre-merge-commit` with `SLH-CLOSES-NO-SPEC` and the trunk does not move; the suite pins that case. Through 2.7.0 both guarantee layers were verified by execution to refuse the same merge while the session layer's warning disappeared, and this bullet recorded that residue; the residue left with the gate. **A decided boundary rather than an open defect, moved to this group 2026-09-02**: a close is what a merge flips, not what a branch is called.
...
A session killed from outside, or one that never reaches its Stop event, fires nothing, and a change staged and never committed is not its question; the git hooks are what refuse the work. Its reason is delivered to the agent, as the bypass deny's is and, since 2.9.0, the scope hook's through `additionalContext` (Design boundaries above).

It is silent where the project root has no `.git` entry of any type, which is the state `/setlist:new` leaves until `/scaffold` creates the repository; 2.6.0 to 2.8.0 refused there, at the end of every new project's first session. The same silence covers an instance stamped below an enclosing repository's top, whose git layer the stamp already reports as not armed, so a spec left unstaged there ends the turn unrefused. A linked worktree (a `.git` file), a corrupt `.git` and a dangling `.git` symlink are still refused, with one exception measured by the 2.9.0 adversarial review and unchanged since 2.6.0: a corrupt `.git` beneath an enclosing repository is not refused, because git walks past it and reads the enclosing repository in its place, whose own git layer then answers for the commit.
...
- **Sideways routes to the trunk.** Content can reach the trunk without a closing merge the merge hook reads: `git cherry-pick` of spec-branch commits (git fires no hook for it) and merges performed on a detached HEAD (no current branch, so no hook sees a trunk target). `git pull <remote> spec/...` used to be on this list as a fetch-and-merge the close gate did not intercept; its merge runs `pre-merge-commit`, which refuses unclosed work with `SLH-CLOSES-NO-SPEC`, measured for 2.8.0.
  **What to do:** Close specs through `/setlist:checkpoint`; read the two remaining routes as the audit's job at push.

<details><summary>The history and the measurements</summary>

Merging a spec branch under a second NAME used to be on this list and left it in 1.0.8, when the close gate began resolving a merge argument to a COMMIT; since 2.8.0 the git hooks judge a merge by the spec rows it closes, which makes the name irrelevant, and the alias bullet in Design boundaries above says so. The remaining routes are named rather than parsed, and a visible edge is more honest than a half-parser that would miss the next spelling. Each of them is caught by the trunk audit below FOR COOPERATING USE, which is why the layers exist: the per-merge hooks refuse at the moment of a merge they can see, and the audit reads history afterwards so it does not care how work arrived. It is not immune to being spelled around either: merge TOPOLOGY crafted to evade it (a chained merge, an octopus parent order) is a separate class, named in its own bullet above, and closing those routes has not been convergent.
...
<a id="rebase-route"></a>
- **`git rebase` onto a spec branch brings that branch's commits to the trunk with no closing merge.** A rebase makes no merge commit, so no merge hook runs and nothing refuses it at the moment you run it.
  **What to do:** Close with `--no-ff`; declare ownership so the audit reads file by file.

<details><summary>The history and the measurements</summary>

Measured: run on the trunk, it leaves the spec's role-path file on the trunk, and the next `git push` is refused with `VIOLATION ... feature code committed directly to main`, remote unchanged. Through 2.7.0 the suite pinned this route through the close gate; since 2.8.0 no case drives it end to end, and the suite's ledger says so rather than claiming a pin. Close a spec with `--no-ff` instead, which is what `/setlist:checkpoint` does. **One qualification, measured by adversarial review and corrected here:** that refusal holds for the bare form. Composed with a compliant single-parent spec close that declares no ownership, the audit's close exemption applies and the push SUCCEEDS; a close that DOES declare is audited file by file instead. See the single-parent declarations bullet above for the exemption and its limits.

</details>

<a id="reset-route"></a>
- **`git reset --hard <spec-branch>` moves the trunk onto unclosed work outright.** Same silence before the push, for the same reason: there is no merge for a merge hook to read.
...
- **`git checkout <spec-branch> -- <path>` copies role-path files onto the trunk without any merge to read.** The pathspec form of checkout writes files from another branch straight into your working tree, and the commit that follows is an ordinary commit: no merge hook runs, and `pre-commit` scans content rather than provenance, so it commits clean.
...
<a id="bash-escape"></a>
- **The Bash escape hatch.** The scope hook watches the file-writing tools (Write, Edit, MultiEdit, NotebookEdit), so a session that writes files through Bash instead (`cat >`, `sed -i`, a heredoc) does not trip it, and `pre-commit` checks what a commit contains rather than which tool wrote the file.
  **What to do:** Read the deny list as a spelling list (`Read(.env)`, `Read(.env.*)`, `Bash(cat .env*)`) and the audit as the backstop.

<details><summary>The history and the measurements</summary>

The same boundary applies to git itself: the bypass deny reads the command it is given, so a git command run through another interpreter (`sh -c '...'`, a backtick, a script file, a heredoc piped into an interpreter) is not a command it can read. Wrapper prefixes that a person actually types (`command`, `env`, `nice`, `nohup`, `exec`, and leading `VAR=value` assignments) are handled by its lexer for `--no-verify` and `-c core.hooksPath`, but NOT for the escape variable, which it reads only at assignment position after nothing but a bare `env` or `export` (the hard-deny bullet names the measured spellings); and that list is not a claim of completeness and cannot be one. The trunk audit below is the designed catch for this whole family: it reads what ended up in your history and does not care how the command was spelled. The stamped deny list is the same kind of thing and its template says so in a `_comment` key: `Read(.env)`, `Read(.env.*)` and, since 2.6.0, `Bash(cat .env*)` stop three spellings of reading a secret and nothing else.

</details>

<a id="advisory-persuades"></a>
- **The scope hook's warning reaches the agent, and the agent may proceed anyway: advisories persuade, hooks refuse.** Since 2.9.0 the scope hook carries its reason in `additionalContext`, which Claude Code adds to the agent's context beside the tool result, and the agent reads it before the write lands. A delivered warning is not a refusal: an agent may attribute the text to the hook, judge the write harmless, and proceed, and the git hooks are what refuse the work at commit, merge and push.
  **What to do:** Read the scope hook's warning as advice the agent has seen; rely on the git hooks' refusals, not on the agent heeding it.

<details><summary>The history and the measurements</summary>

**Until 2.9.0 this was an Upstream condition, "The scope hook's warnings do not reach the agent on current harnesses."** The hook emitted its reason on `permissionDecisionReason`, `systemMessage` and a machine-readable `setlistAdvisory` field, and none of them reached the model when the decision was `allow`, which an advisory's always is. That bullet's own history recorded the plan: if a probe confirmed the sibling field `additionalContext` delivers an advisory to the model, the bullet would become an open limitation whose fix is to use that field. **It skipped that step:** the confirmation and the fix landed in the same release, so the open state never stood on a published list, and the bullet was replaced by this boundary rather than moved.

**How 2.9.0 measured it, at Claude Code 2.1.274.** Two arms, each a scratch instance whose minimal hook emits `additionalContext` carrying a per-run sentinel and an instruction to emit a token, and writes a marker when it fires: one on PreToolUse with `permissionDecision` `allow`, one on PostToolUse. Each was read against a negative twin (the 2.8.0 scope hook wired on the same event, with a marker hook beside it) in the same run, and a control proved a hook's deny reason reaches the session and the deny takes effect. Both arms read SEEN (marker fired, sentinel echoed, token emitted), both twins NOT-SEEN with their markers fired. The scope hook uses PreToolUse, so the warning arrives before the write.

**What the old channels actually did.** In that run the 2.8.0 hook's reason reached the session transcript exactly once, as a system informational notice addressed to the user (`PreToolUse:Write says: setlist [SH-TRUNK-WRITE] ...`), and never in the model's own text: routed to the user's channel, not dropped. That notice carries the `setlist ` prefix only `systemMessage` carried. 2.9.0 removes `systemMessage` from the allow path and keeps `permissionDecisionReason`, which the vendor documents as shown to the user on `allow`.

**How the NOT-SEEN lines of 2.6.0 to 2.8.0 were taken.** The per-release probe of those releases had no marker to prove its hook fired, and its session wrote `src/App.js` into a scratch instance already holding `src/app.js`; on a case-insensitive filesystem that is one file, and the write tool refuses such a write before any PreToolUse hook runs. Those lines therefore could not tell a dropped reason from a hook never dispatched. The 2.9.0 twin measured the drop on a hook proven to have fired.

**Why it persuades rather than refuses.** An agent has been measured attributing an injected advisory correctly to the hook rather than the user, then proceeding on its own judgement that the action was harmless. That is the delivered channel working as designed, and it is why the scope hook stays advisory: the refusal is the git hooks' job.

</details>

<a id="relative-write-path"></a>
- **A relative write path is read against the project root, so the scope hook can miss a role-path write made from a subdirectory.** A write path that is not absolute is resolved against the project root, never the session's working directory, so `a.txt` written from `src/` is read as a file at the root and draws no advisory, where `src/a.txt` does.
  **What to do:** Rely on the push-time trunk audit, which refuses role-path code committed directly to the trunk whichever tool wrote it.

<details><summary>The history and the measurements</summary>

Found in 2.9.0 by enumerating the scope hook's path predicate case by case before its warning began reaching the agent, because a wrong verdict the agent can read is louder than one it cannot. Nine cases: case variance of a role path, a symlinked leaf, a symlinked role directory, a path git would quote, a non-ASCII name, dot segments, a trailing slash on the recorded role, this one, and a docs path beginning with a role's letters (which must stay silent). Three miss: the first two are disclosed under the case-spelling bullet above, and this is the third. Each is pinned in the framework's own suite in its observed direction, so a change to the predicate turns the suite red and this bullet moves with it. Changing the predicate is a separate decision from disclosing it.
