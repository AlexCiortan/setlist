...
- **The scope hook's warnings do not reach the agent on current harnesses.** The scope hook computes its verdict and emits the reason on `permissionDecisionReason`, `systemMessage` and a machine-readable `setlistAdvisory` field, and Claude Code delivers none of them to the model when the decision is `allow`.
  **What to do:** Read the git hooks' refusal messages as the in-session feedback; the maintainer's per-release probe re-checks the harness.

<details><summary>The history and the measurements</summary>

Measured on 2.1.221 with the discriminating control that makes it a harness finding rather than a wiring one: a hook returning `deny` has its reason delivered verbatim, so the hooks are dispatched and the reason is dropped. Consequence: the scope hook's verdict is a machine-readable field that nothing in the shipped plugin reads, not an in-session teaching surface, and the feedback you actually see is the git hooks' refusal at commit or merge time. Through 2.7.0 this bullet named three PreToolUse gates; two of them left in 2.8.0, and the bypass deny beside the scope hook refuses, so its reason is delivered. Filed upstream as a Claude Code bug (PreToolUse allow-path reasons dropped), and re-checked at every release gate by the maintainer's probe, run in the source repository. **Corrected in 2.8.0:** this bullet said the limitation lifts by itself when the harness renders the reasons, and the vendor has since documented the drop on `allow` as intended design, so nothing is coming to lift it. A sibling field on the same event, `additionalContext`, is documented to carry context to the model; Setlist has not measured it, and this bullet stays an upstream condition on documentation alone. **The recorded plan, so the move is not a surprise inside a release:** if a probe confirms that field delivers an advisory to the model, this bullet stops being an upstream condition and becomes an open limitation whose fix is to use that field, which moves one title from Upstream conditions to Open limitations on both layers at once.
